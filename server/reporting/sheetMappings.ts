...
const customerSheet: ReportingSheet = {
  ...
  toRecord: (row) => ({
    ...
    "Customer ID": row.customer_number || row.id,
...
const orderSheet: ReportingSheet = {
  ...
  sheetName: "Orders",
  toRecord: (row) => ({
    "Customer ID": row.customer_number || row.customer_id,
    ...
    customerPhoneSnapshot: row.customer_phone,
