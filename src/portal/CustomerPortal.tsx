...
function CustomerWorkspace({ user }: { user: SessionUser }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const [nextMenu, nextProfile, dashboard, reqs] = await Promise.all([
        loadMenu(),
        loadCustomerProfile(),
        loadCustomerDashboard(),
        loadCustomerVoucherRequests().catch(() => []),
      ]);
      setMenu(nextMenu);
...
function RewardsPanel({
  ...
  onRequestVoucher: (reason?: string) => Promise<void>;
  requestBusy: boolean;
}) {
          ...
          <div className="voucher-request-panel">
            ...
            <label style={{ display: "grid", gap: "var(--space-xs)", fontSize: "0.8125rem", color: "var(--joy-text-secondary)", fontWeight: 600 }}>
              ...
              <input
                ...
                placeholder="e.g. Birthday treat, free latte…"
                value={reason}
              />
            </label>
